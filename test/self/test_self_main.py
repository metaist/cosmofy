...
# pkg
from cosmofy.self import __main__ as self_main


def test_run_shows_usage() -> None:
    """Test run function shows usage when no subcommand."""
    args = self_main.Args()
    with patch.object(self_main.cmd, "show_usage") as mock_show:
        result = self_main.run(args)
        assert result == 0
        mock_show.assert_called_once()


@patch.object(self_main, "cmd")
