...
# pkg
from cosmofy.fs import __main__ as fs_main


def test_run_shows_usage() -> None:
    """Test run function shows usage when no subcommand."""
    args = fs_main.Args()
    with patch.object(fs_main.cmd, "show_usage") as mock_show:
        result = fs_main.run(args)
        assert result == 0
        mock_show.assert_called_once()


@patch.object(fs_main, "cmd")
